...
Update 2026-06-15 / KIM-0104: se agrega `provider=crm action=doctor_pending_report`, un resumen local de pendientes del doctor basado en memoria conversacional reciente, hilos de WhatsApp con `open_items` y acciones programadas pendientes. La meta es separar "correos pendientes" de "mis pendientes" para que el scheduler pueda entregar un estado operativo real al WhatsApp Dubai del doctor a las 8:30 o en pruebas nocturnas sin depender de Codex abierto.

Update 2026-06-05 / KIM-0097: se agrega `provider=google_maps` con `status`, `validate_key`, `find_place`, `geocode`, `reverse_geocode`, `route_distance` y `timezone`. La API key vive en Keychain como `codex.google.maps.api_key` y no se imprime en logs. `find_place` usa Places API (New), `geocode` usa Geocoding API, `route_distance` usa Routes API y `timezone` usa Time Zone API. Las consultas son de lectura, pero pueden generar cargos pequenos en Google Cloud; Kim debe evitar busquedas repetidas innecesarias y guardar resultados utiles en memoria/CRM solo mediante los bridges correspondientes.

Update 2026-06-04 / Kim Live 1.5.69: Zoom queda extendido con `create_and_send_invite`, `get_transcript` y `send_transcript`. `create_and_send_invite` crea reunion real, formatea `join_url`/Meeting ID/passcode y prepara envio por WhatsApp o correo con confirmacion. `get_transcript` consulta `GET /meetings/{meetingId}/transcript`, descarga VTT/texto, limpia timestamps y guarda fuente literal en `BIFROST/MEMORY/zoom_transcripts` y runtime. `send_transcript` prepara envio de resumen local por defecto o transcript completo por email, manteniendo confirmacion para WhatsApp/correo. Requiere OAuth autorizado desde `https://kim.aipeople.app/oauth/zoom/start`, callback exacto `https://kim.aipeople.app/oauth/zoom/callback`, scopes de meetings/user y scope de transcript/grabacion (`recording:read` o granular `cloud_recording:read:meeting_transcript`). Si Zoom no genero cloud recording con audio transcript, Kim debe decirlo y no inventar contenido.

Update 2026-06-04 / Kim Live 1.5.66: se agrega `provider=zoom` con `status`, `auth_url`, `list_users`, `list_meetings` y `create_meeting`. Para la app OAuth normal de Zoom, autorizar desde `https://kim.aipeople.app/oauth/zoom/start` usando callback `https://kim.aipeople.app/oauth/zoom/callback`; requiere Client ID, Client Secret y scopes de meetings/user. `create_meeting` prepara confirmacion antes de crear la reunion real y guarda `meeting_id`/`join_url` en `BIFROST/MEMORY/context/zoom_meetings.jsonl`. Tambien se ajusta WhatsApp: si el doctor pide mandar WhatsApp sin destinatario, se usa por defecto `whatsapp:+971585943726`.

Update 2026-06-02 / Kim Live 1.5.59: se agrega `provider=twilio action=whatsapp_report`, que resume hilos y mensajes entrantes desde `BIFROST/MEMORY/whatsapp_threads` y `whatsapp_thread_index.json` sin fingir un inbox historico directo de Twilio. Tambien se corrige la carga del perfil de voz para elegir la version mas reciente entre `BIFROST` y `~/.kim_live`, evitando que una preferencia nueva quede tapada por un JSON mas viejo.

Update 2026-05-26 / Kim Live 1.5.42: el respaldo BIFROST ahora genera tambien un archivo lateral `BIFROST-<timestamp>-manifest.json`, descargable desde `/api/bifrost-export/manifest`. El manifest declara `BIFROST_EXPORT_MANIFEST`, tamano, SHA-256, cantidad de archivos, expiracion, version de Kim Live y el archivo de arranque recomendado para migracion. El ZIP conserva tambien `BIFROST_EXPORT_MANIFEST.json` adentro.

Update 2026-05-26 / Kim Live 1.5.41: se corrige la descarga BIFROST para navegadores que reintentan o previsualizan el archivo. El token ya no es de un solo uso: dura 30 minutos, requiere sesion activa y permite hasta 5 descargas. Esto evita que Safari/iOS/macOS reciba un `403` en un segundo GET y guarde/abra una respuesta que no es ZIP.

Update 2026-05-26 / Kim Live 1.5.40: se corrige responsive basico despues de agregar el respaldo BIFROST. El panel izquierdo ahora tiene scroll propio en escritorio, `Memoria local` ya no se oculta en movil, la version aparece en landing y app privada, y el header recorta estados largos sin tapar la version.

Update 2026-05-26 / Kim Live 1.5.39: se agrega exportacion segura de BIFROST desde Kim Live. El frontend autenticado puede generar un ZIP temporal con `/api/bifrost-export` solo si valida PIN y frase de seguridad desde Keychain. El enlace `/api/bifrost-export/download` expira en 10 minutos, requiere sesion web activa y se usa una sola vez. El ZIP incluye BIFROST fuente/memoria/docs/CRM, pero no incluye Keychain, `~/.kim_live`, `~/.kim_telegram`, LaunchAgents activos ni sesiones de navegador.

Update 2026-05-25 / Kim Live 1.5.37: `notion/create_page` deja de mandar notas generales al default de `Reuniones` cuando el `notion_default_parent.json` apunta a agenda. Ahora primero consulta `notion_access_inventory.json` y enruta automaticamente segun tipo de contenido: `Reuniones` para eventos, `Wiki` para conocimiento/notas, `Documentos` para reportes o contratos y `Projects` para proyectos. Esto corrige casos como `AI Spirits`, donde los capitulos se estaban creando como reuniones.
